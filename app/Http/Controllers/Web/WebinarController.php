...
class WebinarController extends Controller
{
    ...
    public function getLessonFile(Request $request, $slug, $lesson_id)
    {
        $user = null;
        ...
        $course = Webinar::where('slug', $slug)
            ->where('private', false)
            ->where('status', 'active')
            ->with(['teacher', 'files' => function ($query) {
                $query->orderBy('order', 'asc');
            }])
            ->first();

        if (!empty($course)) {
            $files = File::where('id', $lesson_id)
                ->where('webinar_id', $course->id)->first();

            if (!empty($files)) {
                $canAccess = $course->checkUserHasBought();

                if ($files->accessibility == 'paid' and !$canAccess) {
                    return back();
                }
                ...
                if (!empty($course->files) and count($course->files)) {
                    $nextLesson = File::where('webinar_id',$course->id)->where('id', '>', $files->id)->orderBy('id','asc')->first();
                    $previousLesson = File::where('webinar_id',$course->id)->where('id', '<', $files->id)->orderBy('id','desc')->first();
                }
                ...
                return view(getTemplate() . '.course.files_lesson', $data);
            }
        }
