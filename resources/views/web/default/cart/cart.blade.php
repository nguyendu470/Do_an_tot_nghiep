...
@section('content')
    <section class="cart-banner position-relative text-center">
        ...
        <span class="payment-hint font-20 text-white d-block"> {{$currency . number_format($subTotal,2) . ' ' . trans('cart.for_items',['count' => $carts->count()]) }}</span>
    </section>
                        ...
                        <div class="col-6 col-md-2 col-lg-3 d-flex flex-md-column align-items-center justify-content-center">
                            ...
                            @if(!empty($cart->webinar_id))
                                @if($cart->webinar->getDiscount($cart->ticket))
                                    <span class="text-gray text-decoration-line-through mx-10 mx-md-0">{{ $currency }}{{ number_format($cart->webinar->price, 2, ".", "") + 0 }}</span>
                                    <span class="font-20 text-primary mt-0 mt-md-5 font-weight-bold">{{ $currency }}{{ number_format($cart->webinar->price - $cart->webinar->getDiscount($cart->ticket), 2, ".", "") + 0 }}</span>
                                @else
                                    <span class="font-20 text-primary mt-0 mt-md-5 font-weight-bold">{{ $currency }}{{ number_format($cart->webinar->price, 2, ".", "") + 0 }}</span>
                                @endif

                            @elseif(!empty($cart->reserve_meeting_id))
                                <span class="font-20 text-primary mt-0 mt-md-5 font-weight-bold">{{ $currency }}{{ number_format($cart->reserveMeeting->paid_amount, 2, ".", "") + 0 }}</span>
                            @endif
                        </div>
                ...
                <section class="mt-45">
                    ...
                    <div class="rounded-sm shadow mt-20 pb-20 px-20">
                        ...
                        <div class="cart-checkout-item">
                            ...
                            <span class="font-14 text-gray font-weight-bold">{{ $currency }}{{ number_format($subTotal, 2, ".", "") + 0 }}</span>
                        </div>

                        <div class="cart-checkout-item">
                            <h4 class="text-secondary font-14 font-weight-500">{{ trans('public.discount') }}</h4>
                            <span class="font-14 text-gray font-weight-bold">{{ $currency }}<span id="totalDiscount">{{ number_format($totalDiscount, 2, ".", "") + 0 }}</span></span>
                        </div>
                        ...
                        <div class="cart-checkout-item">
                            ...
                            <span class="font-14 text-gray font-weight-bold">{{ $currency }}<span id="taxPrice">{{ number_format($taxPrice, 2, ".", "") + 0 }}</span></span>
                        </div>

                        <div class="cart-checkout-item border-0">
                            <h4 class="text-secondary font-14 font-weight-500">{{ trans('cart.total') }}</h4>
                            <span class="font-14 text-gray font-weight-bold">{{ $currency }}<span id="totalAmount">{{ number_format($total, 2, ".", "") + 0 }}</span></span>
                        </div>
