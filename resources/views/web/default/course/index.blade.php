                ...
                @if (!empty($activeSpecialOffer))
                    <div class="d-flex align-items-center justify-content-between rounded-lg shadow-xs bg-white p-30">
                        <div class="d-flex flex-column">
                            <strong
                                class="font-16 text-dark-blue font-weight-bold">{{ trans('panel.special_offer') }}</strong>
                        ...
                        <div class="">
                            ...
                            <div id="offerCountDown" class="d-flex time-counter-down"
                                data-day="{{ $remainingTimes['day'] }}" data-hour="{{ $remainingTimes['hour'] }}"
                                data-minute="{{ $remainingTimes['minute'] }}" data-second="{{ $remainingTimes['second'] }}">
    ...
    <section class="container course-content-section">
        <div class="row">
            <div class="col-12 col-lg-8">
                <div class="course-content-body user-select-none">
                            ...
                            <div class="mt-30 d-flex align-items-center">
                                ...
                                <span class="ml-15 font-14 font-weight-500">
                                    @if ($course->isWebinar())
                                        ...
                                        @else
                                            {{ $course->sales_count }}/{{ $course->capacity }}
                                            {{ trans('quiz.students') }}
                                        @endif
                                    @else
                                        {{ trans('public.course_learning_passed', ['percent' => $percent]) }}
                    ...
                    <div class="@if (!$course->isCourse()) mt-35 @else mt-40 pt-40 @endif">
                        ...
                        {{-- form thong tin thong tin khoa học,nội dung, đánh giá --}}
                        <div class="tab-content" id="nav-tabContent">
                            {{-- thong tin khóa học bao gom thong tin khoa học, câu hỏi, bình luận --}}
            ...
            <div class="course-content-sidebar col-12 col-lg-4 mt-25 mt-lg-0">
                <div class="rounded-lg shadow-sm">
                    <div class="course-img {{ $course->video_demo ? 'has-video' : '' }}">
                        ...
                        @if ($course->video_demo)
                            <div id="webinarDemoVideoBtn" data-video-path="{{ config('app_url') . $course->video_demo }}"
                    ...
                    <div class="px-20 pb-30">
                                ...
                                @foreach ($course->tickets as $ticket)
                                    <div class="form-check mt-20">
                                        <input class="form-check-input" @if (!$ticket->isValid()) disabled @endif
                                            type="radio"
                                            data-price='{{ number_format($course->price - ($course->price * $ticket->discount) / 100, 0, ',', '.') }}{{ $currency }}'
                            ...
                            @if ($course->price > 0)
                                <div id="priceBox" class="d-flex align-items-center justify-content-center mt-20">
                                    <span id="realPrice" data-value="{{ $course->price }}"
                                        ...
                                        class="@if (!empty($activeSpecialOffer)) font-16 text-gray text-decoration-line-through mr-15 @else font-30 text-primary @endif">{{ number_format($course->price, 0, ',', '.') }}{{ $currency }}</span>
                                    <span id="priceWithDiscount"
                                        class="font-36 text-primary">{{ !empty($activeSpecialOffer) ? number_format($course->price - ($course->price * $activeSpecialOffer->percent) / 100, 0, ',', '.') . $currency : '' }}</span>
                                </div>
                            @else
                        ...
                        {{-- dòng test khóa học bao gồm, có cho tải xuống không --}}
                        {{-- <div class="mt-35">
                            <strong
                        ...
                        <div class="mt-40 p-10 rounded-sm border row align-items-center favorites-share-box">
                            ...
                            {{-- @if ($course->isWebinar())
                                <div class="col">
                                    <a href="{{ $course->addToCalendarLink() }}" target="_blank"
                                        class="d-flex flex-column align-items-center text-center text-gray">
                            ...
                            {{-- <div class="col">
                                <a href="#" class="js-share-course d-flex flex-column align-items-center text-gray">
                                    ...
                                    <span class="font-12">Share du</span>
                                </a>
                            </div> --}}
                        </div>
                        
                        {{-- báo cáo khóa học này --}}
                ...
                {{-- nếu là lớp học offline thì in tkb, onl thì in chi tiết khóa học --}}
                ...
                <div class="rounded-lg shadow-sm mt-35 px-25 py-20">
                 ...
                 {{--/ end thong tin chi tiết khóa học --}}

                {{-- nếu tổ chức dạy thì in thông tin tổ chức, giáo viên thì in thong tin giáo vien --}}
                {{-- organization --}}
