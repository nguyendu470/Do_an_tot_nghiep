...
@if(!empty($course->files) and $course->files->count() > 0)
    @if(!empty($course->chaps) and $course->chaps->count() > 0)
    <section class="mt-20">
        ...
        <div class="row">
            <div class="col-12">
                <div class="accordion-content-wrapper mt-15" id="chapAccordion" role="tablist" aria-multiselectable="true">
                    @foreach($course->chaps as $chap)
                        @if(!empty($chap->files) and $chap->files->count() > 0)
                        <div class="accordion-row rounded-sm shadow-lg border mt-20 p-15">
                            <div class="row align-items-center" role="tab" id="chap_{{ $chap->id }}">
                                <div class="col-12 d-flex align-items-center" href="#collapseChap{{ $chap->id }}" aria-controls="collapseChap{{ $chap->id }}" data-parent="#chapAccordion" role="button" data-toggle="collapse" aria-expanded="true">
                                    <span class="font-weight-bold text-secondary font-14">{{ $chap->title }}</span>
                                    <div class="icon-position">
                                        {{-- <span class="mr-15 d-flex"><i data-feather="plus" width="20" height="20" class="text-gray"></i></span>
                                        <span class="mr-15 d-flex"><i data-feather="minus" width="20" height="20" class="text-gray"></i></span> --}}
                                    </div>
                                </div>
                            </div>
                            <div id="collapseChap{{ $chap->id }}" aria-labelledby="chap_{{ $chap->id }}" class=" collapse" role="tabpanel">
                                <div class="panel-collapse">
                                    {{-- Files --}}
                                    <div class="mt-15">
                                        <div class="row">
                                                            ...
                                                            <div class="row align-items-center" role="tab" id="files_{{ $file->id }}">
                                                                <div class="col-9 col-md-6 d-flex align-items-center" href="#collapseFiles{{ $file->id }}" aria-controls="collapseFiles{{ $file->id }}" data-parent="#filesAccordion" role="button" data-toggle="collapse" aria-expanded="true">
                                                                    ...
                                                                    @if($file->accessibility == 'paid')
                                                                        @if(!empty($user) and $hasBought)
                                                                            <a href="{{ $course->getUrl() }}/lessons/{{ $file->id }}/watch" target="_blank" class="mr-15" data-toggle="tooltip" data-placement="top" title="{{ trans('public.read') }}">
                                                                ...
                                                                <div class="col-3 d-flex justify-content-end">
                                                                    @if($file->accessibility == 'paid')
                                                                        @if(!empty($user) and $hasBought)
                                                                            <a href="{{ $course->getUrl() }}/lessons/{{ $file->id }}/watch" target="_blank" class="course-content-btns btn btn-sm btn-primary flex-grow-1">
                                                                                {{ trans('public.read') }}
                                                                            </a>
                                                                        @else
        ...
        <div class="row">
            <div class="col-12">
                <div class="accordion-content-wrapper mt-15" id="chapAccordionLessons" role="tablist" aria-multiselectable="true">
                    @foreach($course->chaps as $chap)
                        @if(!empty($chap->textLessons) and $chap->textLessons->count() > 0)
                        <div class="accordion-row rounded-sm shadow-lg border mt-20 p-15">
                            <div class="row align-items-center" role="tab" id="chapLessons_{{ $chap->id }}">
                                <div class="col-12 d-flex align-items-center" href="#collapseChapLessons{{ $chap->id }}" aria-controls="collapseChapLessons{{ $chap->id }}" data-parent="#chapAccordionLessons" role="button" data-toggle="collapse" aria-expanded="true">
                                    <span class="font-weight-bold text-secondary font-14">{{ $chap->title }}</span>
                                    <div class="icon-position">
                                        {{-- <span class="mr-15 d-flex"><i data-feather="plus" width="20" height="20" class="text-gray"></i></span>
                                        <span class="mr-15 d-flex"><i data-feather="minus" width="20" height="20" class="text-gray"></i></span> --}}
                            ...
                            <div id="collapseChapLessons{{ $chap->id }}" aria-labelledby="chapLessons_{{ $chap->id }}" class=" collapse" role="tabpanel">
                                <div class="panel-collapse">
                                    {{-- TextLessons --}}
                                    <div class="mt-15">
                                        <div class="row">
                                        ...
                                        <div class="row">
                                            <div class="col-12">
                                                <div class="accordion-content-wrapper mt-15" id="textLessonsAccordion" role="tablist" aria-multiselectable="true">
                                                    @foreach($chap->textLessons as $textLesson)
                                                        <div class="accordion-row rounded-sm shadow-lg border mt-20 p-15">
